...
- **Command:** Create a Docker environment from the open requirements file by running:
  ```docker env create -f YOUR-REQUIREMENTS.YML```
...
- **Command:** Activate an existing Docker environment with:
  ```docker activate YOUR-ENVIRONMENT```
- **VS Code Command Palette:** `>Docker Wingman: Activate Docker Environment`

### Writing Requirements Files
- **Command:** Export the active Docker environment to a requirements file with:
  ```docker env export > YOUR_REQUIREMENTS_FILE```
- **VS Code Command Palette:** `>Docker Wingman: Write a requirements Dockerfile from the active Docker Environment`

### Deleting Environments
- **Command:** Remove an existing Docker environment by first deactivating, then removing it with:
  ```
  docker deactivate
  docker env remove -n YOUR-ENVIRONMENT
  ```
- **VS Code Command Palette:** `>Docker Wingman: Delete Docker Environment`
